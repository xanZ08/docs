...
# Navigation
In this article we will cover how to do navigation in a NativeScript application using Angular.
...
Create a `RouterConfig` object that maps paths to components and parameters:
...
NativeScript apps consist of pages that represent the separate application screens. Pages are instances of the [`Page`](http://docs.nativescript.org/api-reference/classes/_ui_page_.page.html) class. Page navigation integrates with the native navigation elements on the current platform (ex. the **Back** button in Android or the **NavigationBar** in iOS). 
...
* `router-outlet`: replaces the content of the outlet with a different component. It is the default outlet that comes from Angular 2.
...
It is possible to nest the `<router-outlet>` component inside `<page-router-outlet>` or another `<router-outlet>`.
...
You can navigate back using the `back()` method of the `RouterExtensions`:

{%snippet router-back%}

You can also navigate back to the previous page using the `backToPreviousPage()` method:
...
Or you can use the `RouterExtensions` class:
...
> **Note**: Currently, there is no way to prevent user-initiated back navigation - trying to apply guards in such scenarios is not supported.
